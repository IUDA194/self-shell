...
Для каждых обоев автоматически строится собственная палитра. Цветной круг на
карточке открывает отдельное окно «Настройки» на вкладке «Внешний вид», где
доступен ручной выбор: Auto, Default, Kanagawa, Gruvbox или Nord.
Выбор сохраняется отдельно для каждого файла в
`~/.config/self-shell/wallpaper-themes.json`. Текущая тёплая палитра доступна
как Default и используется, пока активная схема ещё не была создана.

Во вкладке «Система» можно настроить часовой пояс и автообновление оболочки с
GitHub: выключить его, проверять при старте или раз в указанное число дней.
Обновление выполняется только fast-forward и не перезаписывает локальные
изменения.
